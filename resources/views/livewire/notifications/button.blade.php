...
>
    ...
    <svg class="h-8 w-8 mt-2 mr-10 fill-black dark:fill-white" stroke="currentColor" viewBox="0 0 24 24"
         xmlns="http://www.w3
    .org/2000/svg">
    ...
    @if ($unread->count() > 0)
        <span aria-label="unread count" class="absolute top-0 left-0 aspect-square max-h-fit rounded-full
            border-2 bg-red-500 ml-4 px-1.5 shadow leading-2 text-white font-semibold text-sm">
                {{ $unread->count() }}
            </span>
    @endif
</button>
